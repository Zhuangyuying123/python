...
def process_data(data):
    """数据预处理"""
    # 选择需要的列
    data = data[['建造年份', '房价', '在售房数', '建筑材质','区划']].copy()
...
def main():
    ...
    # 预测训练集并输出分类报告和混淆矩阵
    train_predicted = rf.predict(x_train_w)  # 使用随机森林模型进行预测
    train_report = classification_report(y_train_w, train_predicted, output_dict=True)
    print("Train Classification Report:")
    print(train_report)
    cm_plot(y_train_w, train_predicted, '92-随机森林-训练集混淆矩阵.jpg')  # 保存混淆矩阵为 JPG 文件

    # 保存训练集的分类报告
    pd.DataFrame(train_report).transpose().to_csv('92-随机森林-训练集分类报告.csv')

    # 预测测试集并输出分类报告和混淆矩阵
    test_predicted = rf.predict(x_test_w)  # 使用随机森林模型进行预测
    test_report = classification_report(y_test_w, test_predicted, output_dict=True)
    print("Test Classification Report:")
    print(test_report)
    cm_plot(y_test_w, test_predicted, '92-随机森林-测试集混淆矩阵.jpg')  # 保存混淆矩阵为 JPG 文件

    # 保存测试集的分类报告
    pd.DataFrame(test_report).transpose().to_csv('92-随机森林-测试集分类报告.csv')
